...
connection1=psycopg2.connect(host='3.108.122.116',database='staging_db', user='staging_user', password='Staging-2023',port='5432')
cursor1 = connection1.cursor()

# Establish connection to the reporting database
connection2=psycopg2.connect(host='3.108.122.116',database='reporting_db', user='reporting_user', password='Reporting-2023',port='5432')
cursor2 = connection2.cursor()

product_list=['date', 'agent_id', 'emp_name', 'call_attended', 'login_duration', 'total_talk_time', 'total_break', 'total_wrap_time', 'total_hold_time', 'bio', 'lunch', 'tea', 'total_idle_time', 'avg_time_hold_daily', 'avg_wrap_time_daily', 'avg_talk_time_daily', 'avg_handling_time_daily', 'utilization', 'occupancy', 'wrap_perc', 'hold_perc','emp_id']
product_csat_list=['date', 'agent_id', 'emp_name', 'call_attended','emp_id','login_duration', 'total_talk_time', 'total_break', 'total_wrap_time', 'total_hold_time', 'bio', 'lunch', 'tea', 'total_idle_time', 'avg_time_hold_daily', 'avg_wrap_time_daily', 'avg_talk_time_daily', 'avg_handling_time_daily', 'utilization', 'occupancy', 'wrap_perc', 'hold_perc','ftr','total_calls','survey_ivr','csat','dsat','neutral','response_count','great_service','csat_per','dsat_per', 'neutral_per', 'great_service_per','first_call_resolution','ivr_transfer','taged','ans']
...
def product_sdb_to_rdb():
    ...
    for x in pd.date_range(startdate, enddate):
        ...
        query = f'''SELECT {final_columns} from agent_info_sdb where date='{x.date()}' and emp_id is not null;'''
        ...
        for i, row in df.iterrows():
            value = row.values
            try:
                query = f'''INSERT INTO product_rdb ({final_columns}) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);'''
...
product_exportcall_list=['date', 'user', 'full_name', 'call_date', 'status', 'campaign_id', 'query_typelevel_1', 'ans', 'tagging', 'ftr', 'call_transferred_to_survey_ivr', 'survey_ivr', 'emp_id', 'extracted_time', 'extracted_interval', 'count', 'abandon', 'outbound', 'phone_number', 'cust_talk_sec', 'customer_hold_time', 'wrap_up_time', 'queue_seconds', 'sl', 'ht','query_type_level_2','subquery_type_level_2','lead_id','customer_number','repeat_daily','repeat_7days','repeat_3days','csat','quality_score','survey_feedback']
...
def exportcall_sdb_to_rdb():
    ...
    for date in pd.date_range(startdate, enddate):
        ...
        # Check if the counts of data in the source SDB and target RDB are equal for the current date
            ...
            cursor2.execute(query)
            connection2.commit()
            query = f'''select process_final('{date.date()}');'''
            cursor1.execute(query)
            connection1.commit()          
            ...
            query = f'''select {final_columns} from tem_proces;'''
            cursor1.execute(query)
            column_names = [desc[0] for desc in cursor1.description]
            df = pd.DataFrame(cursor1.fetchall(), columns=column_names)           
            # Convert the 'date' column to a datetime object and format it as '%Y-%m-%d'
            ...
            for i, row in df.iterrows():
                ...
                try:
                    ...
                    connection2.commit()      
        # Print the current date being processed
            update_query = f'''update exportcall_rdb set team_lead=fhpl_emp_details_rdb.team_lead from fhpl_emp_details_rdb where exportcall_rdb.emp_id=fhpl_emp_details_rdb.employeeid and exportcall_rdb.date='{date.date()}';'''
            cursor2.execute(update_query)
            connection2.commit()
            query = '''drop table tem_proces;'''
            cursor1.execute(query)
            connection1.commit()
            print(date.date())  
...
csat_exportcall_column_list=['date','user','full_name','ftr','total_calls','survey_ivr','csat','dsat','neutral','response_count','great_service','user_full_name','emp_id','taged','ans']
...
def csat_sdb_to_rdb():
    ...
    for x in pd.date_range(startdate, enddate):
        ...
        cursor2.execute(query)       
        final_columns = ','.join(['"{0}"'.format(item) for item in csat_exportcall_column_list]) 
        ...
        query = f'''SELECT {final_columns} from temp_csat;'''
        cursor1.execute(query)       
        ...
        for i, row in df.iterrows():
            value = row.values
            try:
                query = f'''INSERT INTO csat_rdb ({final_columns}) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);'''
                cursor2.execute(cursor2.mogrify(query, value))
                connection2.commit()
            except Exception as e:
                cursor2.execute("ROLLBACK")
...
def product_csat_rdb():
    ...
    for x in pd.date_range(startdate, enddate):
        ...
        for i, row in df.iterrows():
            value = row.values
            try:
                query = f'''INSERT INTO product_csat_rdb ({final_column}) VALUES (%s,%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);'''
                cursor2.execute(cursor2.mogrify(query, tuple(value)))
                connection2.commit()
            except Exception as e:
                cursor2.execute("ROLLBACK")
        ...
        cursor2.execute(query)
        connection2.commit()
        query = f'''select updating_product_tl_and_manager_names('{x.date()}');'''
        cursor2.execute(query)
        connection2.commit()
    # Close the cursors and connections to the databases
    cursor1.close()
...
def shrinkage_function():
    ...
    for x in pd.date_range(startdate, enddate):
        ...
        query = f'''SELECT date, "interval", roster_emp_count, extracted_interval, count, ans, sl, sl_per, abn, aht, asa, forecast,repeat_daily,repeat_3days,repeat_7days from interval_wise_table;'''
        cursor1.execute(query)      
        # Extract column names from the cursor's description and create a DataFrame
        column_names = [desc[0] for desc in cursor1.description]
        df = pd.DataFrame(cursor1.fetchall(), columns=column_names)    
        ...
        for i, row in df.iterrows():
            value = row.values
            try:
                query = '''INSERT INTO interval_wise_rdb (date, "interval", roster_emp_count, extracted_interval, count, ans, sl, sl_per, abn, aht, asa, forecast,repeat_daily,repeat_3days,repeat_7days) VALUES (%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s);'''
                cursor2.execute(cursor2.mogrify(query, value))
                connection2.commit()
            except Exception as e:
                cursor2.execute("ROLLBACK")
