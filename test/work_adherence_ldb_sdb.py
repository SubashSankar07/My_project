...
connection_source = psycopg2.connect(host='3.108.122.116',database='landing_db', user='landing_user', password='Landing-2023',port='5432')
cursor_source = connection_source.cursor()
connection_dest=psycopg2.connect(host='3.108.122.116',database='staging_db', user='staging_user', password='Staging-2023',port='5432')
...
def emp_details_sdb_function():
    ...
    df['team_lead'] = df['team_lead'].apply(lambda x : x.split('.')[0])
...
agent_roster_ldb_sdb>>login_logout_ldb_sdb>>attrition_tracker_ldb_sdb>>emp_details_ldb_sbd